...
Check ([Runtime.InteropServices.Marshal]::SizeOf([FolderSailMarketingConsumerDisplay.Mode]::new()) -eq 220 -and
    [Runtime.InteropServices.Marshal]::OffsetOf([FolderSailMarketingConsumerDisplay.Mode],'dmPelsWidth').ToInt32() -eq 172 -and
    [Runtime.InteropServices.Marshal]::SizeOf([FolderSailMarketingConsumerDisplay.DisplayDevice]::new()) -eq 840) 'Native Unicode display structure layout differs.'
foreach($flag in @(1,0x100,0x200,0x10000000)){
    $failed=$false;try{$null=[FolderSailMarketingConsumerDisplay.DisplayModes]::Change('not-a-device',[FolderSailMarketingConsumerDisplay.Mode]::new(),$flag)}catch{$failed=$_.Exception.Message -match 'Only native test and dynamic change'}
...
Check ((Get-FolderSailMarketingConsumerModeChoice @((Mode 1280 720),$desired,(Mode 2560 1440))).dmPelsWidth -eq 1920) 'A supported 1080p mode was not selected.'
foreach($modes in @(@((Mode 1024 768)),@((Mode 1920 1080 16)),@((Mode 4000 3000)))){
    $failed=$false;try{$null=Get-FolderSailMarketingConsumerModeChoice $modes}catch{$failed=$true};Check $failed 'Absent/unsafe/oversized supported mode was accepted.'
}
...
function Get-FolderSailMarketingConsumerDisplayState([string]$Device){@{device='fixture-primary';current=$script:current;modes=@($original,$desired)}}
function Set-FolderSailMarketingConsumerDisplayMode([string]$Device,$Mode,[int]$Flags){$script:calls.Add($Flags);if($Flags -eq 2){if($script:rejectTest){return -2};return 0};$script:current=$Mode;if($script:partialApplyFailure -and $Mode.dmPelsWidth -eq 1920){return -1};return 0}
$state=@{displayOriginalMode=$null;displayDevice=$null;displayRestoreRequired=$false;displayEvidence=$null}
Start-FolderSailMarketingConsumerDisplay $state
...
Restore-FolderSailMarketingConsumerDisplay $state
...
$state=@{displayOriginalMode=$null;displayDevice=$null;displayRestoreRequired=$false;displayEvidence=$null}
$failed=$false;try{Start-FolderSailMarketingConsumerDisplay $state}catch{$failed=$true}
...
function Set-FolderSailMarketingConsumerDisplayMode([string]$Device,$Mode,[int]$Flags){return 0}
function Start-Sleep { param($Milliseconds) }
$state.displayRestoreRequired=$true;$script:current=$original
$failed=$false;try{Restore-FolderSailMarketingConsumerDisplay $state}catch{$failed=$_.Exception.Message -match 'was not restored'}
