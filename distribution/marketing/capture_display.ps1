...
function Add-FolderSailMarketingConsumerDisplayTypes {
    if('FolderSailMarketingConsumerDisplay.DisplayModes' -as [type]){return}
    Add-Type -TypeDefinition @'
using System;
using System.Collections.Generic;
using System.ComponentModel;
using System.Runtime.InteropServices;
namespace FolderSailMarketingConsumerDisplay {
...
function Convert-FolderSailMarketingConsumerMode($Mode){
...
function Get-FolderSailMarketingConsumerModeChoice($Modes){
    $candidates=@($Modes|Where-Object {$_.dmBitsPerPel -eq 32 -and $_.dmPelsWidth -ge 1920 -and $_.dmPelsHeight -ge 1080 -and
...
function Get-FolderSailMarketingConsumerDisplayState([string]$Device){
    Add-FolderSailMarketingConsumerDisplayTypes
    if(-not $Device){$Device=[FolderSailMarketingConsumerDisplay.DisplayModes]::Primary()}
    return @{device=$Device;current=[FolderSailMarketingConsumerDisplay.DisplayModes]::Current($Device);modes=[FolderSailMarketingConsumerDisplay.DisplayModes]::Supported($Device)}
}
function Set-FolderSailMarketingConsumerDisplayMode([string]$Device,$Mode,[int]$Flags){
    return [FolderSailMarketingConsumerDisplay.DisplayModes]::Change($Device,$Mode,$Flags)
}

function Start-FolderSailMarketingConsumerDisplay($State){
    $snapshot=Get-FolderSailMarketingConsumerDisplayState
    $State.displayOriginalMode=$snapshot.current;$State.displayDevice=$snapshot.device
    $State.displayEvidence=[ordered]@{purpose='actual native display only';device=$snapshot.device;before=(Convert-FolderSailMarketingConsumerMode $snapshot.current);
        supported_modes=@($snapshot.modes|ForEach-Object {Convert-FolderSailMarketingConsumerMode $_});selected=$null;test_result=$null;apply_result=$null;after=$null;
    ...
    if($snapshot.current.dmPelsWidth -ge 1920 -and $snapshot.current.dmPelsHeight -ge 1080 -and $snapshot.current.dmBitsPerPel -eq 32){
        $State.displayEvidence.after=Convert-FolderSailMarketingConsumerMode $snapshot.current;return
    }
    $selected=Get-FolderSailMarketingConsumerModeChoice $snapshot.modes;$State.displayEvidence.selected=Convert-FolderSailMarketingConsumerMode $selected
    $test=Set-FolderSailMarketingConsumerDisplayMode $snapshot.device $selected 2;$State.displayEvidence.test_result=$test
    ...
    $apply=Set-FolderSailMarketingConsumerDisplayMode $snapshot.device $selected 0;$State.displayEvidence.apply_result=$apply
    if($apply -ne 0){throw "Native dynamic display change failed ($apply)."}
    $deadline=[DateTime]::UtcNow.AddSeconds(5)
    do{
        $after=(Get-FolderSailMarketingConsumerDisplayState $snapshot.device).current
    ...
    $State.displayEvidence.after=Convert-FolderSailMarketingConsumerMode $after
...
function Restore-FolderSailMarketingConsumerDisplay($State){
    if(-not $State.displayOriginalMode){return}
    if($State.displayRestoreRequired){
        $result=Set-FolderSailMarketingConsumerDisplayMode $State.displayDevice $State.displayOriginalMode 0;$State.displayEvidence.restore_result=$result
        if($result -ne 0){throw "Native display restoration failed ($result)."}
    }
    $deadline=[DateTime]::UtcNow.AddSeconds(5)
    do{
        $observed=Convert-FolderSailMarketingConsumerMode (Get-FolderSailMarketingConsumerDisplayState $State.displayDevice).current
