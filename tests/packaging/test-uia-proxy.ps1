...
Initialize-FileQuayUiaReplayCollision @('ClientSettings')
...
'@
 ...
 try{$clientDoubleAssembly=[Runtime.Loader.AssemblyLoadContext]::Default.LoadFromStream($stream)}finally{$stream.Dispose()}
 $clientDouble=$clientDoubleAssembly.GetType('System.Windows.Automation.ClientSettings',$true)
 Require ([object]::ReferenceEquals($clientDouble.Assembly,$clientDoubleAssembly) -and $clientDouble -ne ('System.Windows.Automation.ClientSettings' -as [type])) 'Fixture observations did not retain the exact independent client assembly/type.'
 ...
 Require ($null -ne $caught -and $clientDouble::Calls -eq 1 -and -not $r.registered) 'Registration failure was swallowed, retried or marked registered.'
 ...
 $clientDouble::BreakDiagnostic=$true
 ...
 Require ($caught.Exception.Message -like '*original-registration-failure*' -and -not $r.registered -and
  $r.registration_exception_error -like '*diagnostic-format-failure*' -and $clientDouble::Calls -eq 2) 'Diagnostic formatting failure masked primary registration refusal or caused replay.'
 ...
 $clientDouble::BreakDiagnostic=$false
 $clientDouble::WalkStack=$true
 $directFailure=$null
 try{$clientDouble::RegisterClientSideProviderAssembly([Reflection.AssemblyName]::new('UIAutomationClientsideProviders'))}catch{$directFailure=$_}
 Require ($null -ne $directFailure -and $clientDouble::MissingReflectedType) 'Direct PowerShell call did not reproduce the actual WPF caller-frame defect.'
 ...
 Require (-not $clientDouble::MissingReflectedType -and
  $clientDouble::CallingType -ceq 'FileQuayQualification.UiaProxyRegistration' -and
 ...
 foreach($drift in @('source','type','client')) {
  ...
  $callsBefore=$clientDouble::Calls;$r=@{registered=$false}
  Reject {Register-FileQuayUiaProxy $r} "typed caller $drift binding drift"
  Require (-not $r.registered -and -not $r.registration_call.entered -and $clientDouble::Calls -eq $callsBefore) 'Unbound typed caller reached the public API or changed registration acceptance.'
 }
 $script:FileQuayUiaRegistrationIdentity=$originalBinding
