...
function Identity([string]$Name) {
    @{path=(Join-Path $hostRoot ($Name+'.dll'));name=$Name;version='10.0.0.0';public_key_token='31bf3856ad364e35';
...
$checks=1
foreach ($mode in @('foreign-path','wrong-name','wrong-version','wrong-token','wrong-culture','unsigned','foreign-signer','wrong-company','missing-hash','empty-file','foreign-client')) {
    $c=@{}+$client;$p=@{}+$proxy
    switch ($mode) {
