...
function Reset {
 ...
 $filename.element|Add-Member ScriptMethod GetCurrentPattern {param($Id)$script:reads++;$script:events.Add('filename-read');if([FileQuayQualification.ConsumerInput]::Texts -gt 0){$script:pattern.Current.Value=$(if($script:wrongValue -or $script:reads -lt (2+$script:valueDelay)){'FolderSail-receipts.csv'}else{[FileQuayQualification.ConsumerInput]::LastText})};$script:pattern}
...
function Invoke-FileQuayWorkflowAction($Ui,$Binding,$Action,$Value=$null){
 ...
 if($Binding.element.Current.AutomationId -ceq '1001' -and $Action -ceq 'Value'){throw 'Filename must use ordinary native input, not ValuePattern.SetValue.'}
...
Require ([object]::ReferenceEquals($result,$custom) -and [FileQuayQualification.ConsumerInput]::Spaces -eq 1 -and $reads -eq 2 -and [FileQuayQualification.ConsumerInput]::Texts -eq 1) 'Native Save did not reach exact app confirmation after readback.';$checks++
Require (($events -join ',') -ceq 'export-invoke,filename-read,focus-1001,filename-read,focus-1,original-file-proof') 'Save sequence changed.';$checks++
...
Reset;$valueDelay=2
$result=Open-FileQuayWorkflowExportConfirmation $ui $fixture
Require ($reads -eq 4 -and [FileQuayQualification.ConsumerInput]::Texts -eq 1 -and [FileQuayQualification.ConsumerInput]::Spaces -eq 1) 'Asynchronous filename delivery must converge without replaying input.';$checks++
Reset;$wrongConfirmation=$true
Reject {Open-FileQuayWorkflowExportConfirmation $ui $fixture} 'exact selected CSV path'
Require ([FileQuayQualification.ConsumerInput]::Texts -eq 1 -and [FileQuayQualification.ConsumerInput]::Spaces -eq 1 -and -not $events.Contains('original-file-proof')) 'Correct field readback must not accept a different actual picker result.';$checks++
foreach($change in @(@('AutomationId','2'),@('ClassName','ComboBox'),@('ControlType','Pane'),@('NativeWindowHandle',0),@('ProcessId',999),@('IsKeyboardFocusable',$false),@('IsEnabled',$false),@('IsOffscreen',$true))){
 Reset;$filename.element.Current.($change[0])=$change[1]
 Reject {Set-FileQuayWorkflowPickerFilename $ui $filename $fixture.csv} 'filename control'
 Require ([FileQuayQualification.ConsumerInput]::Texts -eq 0 -and $focusRequests -eq 0) 'Unproved filename field received focus or text.';$checks++
}
Reset;$focusDelay=3
Set-FileQuayWorkflowPickerFilename $ui $filename $fixture.csv
Require ($focusRequests -eq 1 -and [FileQuayQualification.ConsumerInput]::Texts -eq 1) 'Filename focus must converge after one request.';$checks++
Reset;$focusDelay=100
Reject {Set-FileQuayWorkflowPickerFilename $ui $filename $fixture.csv} 'filename control'
Require ($focusRequests -eq 1 -and $focusReads -eq 20 -and [FileQuayQualification.ConsumerInput]::Texts -eq 0) 'Missing filename focus must refuse without text.';$checks++
Reset;$focusDelay=4;$lateDrift=@{key='NativeWindowHandle';value=999}
Reject {Set-FileQuayWorkflowPickerFilename $ui $filename $fixture.csv} 'filename control'
Require ($focusRequests -eq 1 -and $focusReads -eq 2 -and [FileQuayQualification.ConsumerInput]::Texts -eq 0) 'Native filename HWND changed during focus wait.';$checks++
